...
apt install -y curl ca-certificates build-essential python3 libxml2-dev uuid-dev libssl-dev cpio libbz2-dev make patch git zlib1g-dev unzip
...
# Build & install clang
UNATTENDED=1 ./build_apple_clang.sh
make -C /tmp/osxcross/build/clang-17/build_stage2 install

# Build and place the osxcross
OUT_DIR=/opt/osxcross
