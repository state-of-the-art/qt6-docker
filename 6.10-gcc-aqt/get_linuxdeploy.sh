#!/bin/sh -xe
# Script to install linuxdeploy with qt plugin

[ "$LINUXDEPLOY_GIT" ] || LINUXDEPLOY_GIT="https://github.com/linuxdeploy/linuxdeploy.git"
[ "$LINUXDEPLOY_COMMIT" ] || LINUXDEPLOY_COMMIT="LINUXDEPLOY_COMMIT IS NOT SET"
[ "$LINUXDEPLOY_QT_GIT" ] || LINUXDEPLOY_QT_GIT="https://github.com/linuxdeploy/linuxdeploy-plugin-qt.git"
[ "$LINUXDEPLOY_QT_COMMIT" ] || LINUXDEPLOY_QT_COMMIT="LINUXDEPLOY_QT_COMMIT IS NOT SET"
[ "$LINUXDEPLOY_APPIMAGE_GIT" ] || LINUXDEPLOY_APPIMAGE_GIT="https://github.com/linuxdeploy/linuxdeploy-plugin-appimage.git"
[ "$LINUXDEPLOY_APPIMAGE_COMMIT" ] || LINUXDEPLOY_APPIMAGE_COMMIT="LINUXDEPLOY_APPIMAGE_COMMIT IS NOT SET"

[ "$APPIMAGETOOL_VERSION" ] || APPIMAGETOOL_VERSION="APPIMAGETOOL_VERSION IS NOT SET"
[ "$APPIMAGETOOL_SHA256" ] || APPIMAGETOOL_SHA256="APPIMAGETOOL_SHA256 IS NOT SET"
[ "$APPIMAGETOOL_URL" ] || APPIMAGETOOL_URL="https://github.com/AppImage/appimagetool/releases/download/$APPIMAGETOOL_VERSION/appimagetool-x86_64.AppImage"

# Init the package system
apt update

echo
echo '--> Save the original installed packages list'
echo

dpkg --get-selections | cut -f 1 > /tmp/packages_orig.lst

echo
echo '--> Install the required packages to install linuxdeploy'
echo

apt install -y git curl libboost-filesystem-dev libboost-regex-dev cimg-dev wget patchelf nlohmann-json3-dev build-essential

echo
echo '--> Download & install the linuxdeploy'
echo

git clone "$LINUXDEPLOY_GIT" /tmp/linuxdeploy
git -C /tmp/linuxdeploy checkout "$LINUXDEPLOY_COMMIT"
git -C /tmp/linuxdeploy submodule update --init --recursive
git clone "$LINUXDEPLOY_QT_GIT" /tmp/linuxdeploy-plugin-qt
git -C /tmp/linuxdeploy-plugin-qt checkout "$LINUXDEPLOY_QT_COMMIT"
git -C /tmp/linuxdeploy-plugin-qt submodule update --init --recursive
git clone "$LINUXDEPLOY_APPIMAGE_GIT" /tmp/linuxdeploy-plugin-appimage
git -C /tmp/linuxdeploy-plugin-appimage checkout "$LINUXDEPLOY_APPIMAGE_COMMIT"
git -C /tmp/linuxdeploy-plugin-appimage submodule update --init --recursive

cmake /tmp/linuxdeploy -B /tmp/linuxdeploy-build -G Ninja -DCMAKE_INSTALL_PREFIX=/usr/local -DCMAKE_BUILD_TYPE=Release -DUSE_CCACHE=OFF
cmake --build /tmp/linuxdeploy-build

cmake /tmp/linuxdeploy-plugin-qt -B /tmp/linuxdeploy-plugin-qt-build -G Ninja -DCMAKE_INSTALL_PREFIX=/usr/local -DCMAKE_BUILD_TYPE=Release -DUSE_CCACHE=OFF
cmake --build /tmp/linuxdeploy-plugin-qt-build

cmake /tmp/linuxdeploy-plugin-appimage -B /tmp/linuxdeploy-plugin-appimage-build -G Ninja -DCMAKE_INSTALL_PREFIX=/usr/local -DCMAKE_BUILD_TYPE=Release -DUSE_CCACHE=OFF
cmake --build /tmp/linuxdeploy-plugin-appimage-build

mkdir -p /usr/local/bin
mv /tmp/linuxdeploy-build/bin/linuxdeploy /usr/local/bin
mv /tmp/linuxdeploy-plugin-qt-build/bin/linuxdeploy-plugin-qt /usr/local/bin
mv /tmp/linuxdeploy-plugin-appimage-build/src/linuxdeploy-plugin-appimage /usr/local/bin

echo
echo '--> Download & install the appimagetool'
echo

echo "$APPIMAGETOOL_SHA256 -" > sum.txt && curl -fLs "$APPIMAGETOOL_URL" | tee /usr/local/bin/appimagetool.AppImage | sha256sum -c sum.txt; rm -f sum.txt
cat - <<\EOF | tee /usr/local/bin/appimagetool
#!/bin/sh
# AppImage can't be run directly in the docker container (no fuse support), so unpacking and running it
appimagetool.AppImage --appimage-extract-and-run "$@"
EOF
chmod +x /usr/local/bin/appimagetool /usr/local/bin/appimagetool.AppImage

echo
echo '--> Restore the packages list to the original state'
echo

dpkg --get-selections | cut -f 1 > /tmp/packages_curr.lst
grep -Fxv -f /tmp/packages_orig.lst /tmp/packages_curr.lst | xargs apt remove -y --purge

# Complete the cleaning

apt -qq clean
rm -rf /var/lib/apt/lists/* /tmp/linuxdeploy*
