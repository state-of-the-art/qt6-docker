...
aqt install-src -O "$QT_PATH" linux "$QT_VERSION" --archives qttools # Contains macdeployqt
...
cat - <<\EOF | patch -p 1 "$QT_MACOS/../Src/qttools/src/macdeployqt/shared/shared.cpp"
--- a/src/macdeployqt/shared/shared.cpp 2021-10-21 02:07:18.000000000 -0700
+++ b/src/macdeployqt/shared/shared.cpp 2022-01-31 14:21:29.871498238 -0800
@@ -411,6 +411,10 @@
         CFRelease(bundle);
     }
     CFRelease(bundleURL);
...
"$QT_MACOS/../gcc_64/bin/qt-cmake" -S "$QT_MACOS/../Src/qttools" -B /tmp/macdeployqt-build \
    -DQT_FEATURE_macdeployqt=ON -DQT_FEATURE_linguist=OFF -DQT_FEATURE_assistant=OFF \
    -DQT_FEATURE_clang=OFF -DQT_FEATURE_qtplugininfo=OFF -DQT_FEATURE_qdbus=OFF \
...
cp -a "$QT_MACOS/../gcc_64/lib"/libQt6Core.so* "$QT_MACOS/../gcc_64/lib"/libicu* /usr/local/lib/
