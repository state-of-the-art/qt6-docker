...
mkdir -p $root_dir/build/qt6-build ; cd $root_dir/build/qt6-build
# Config options useful doc: https://doc-snapshots.qt.io/qt6-dev/configure-options.html
for abi in armeabi-v7a arm64-v8a x86 x86_64; do
    ../qt-src/configure -prefix "$QT_PATH/$abi" -nomake examples -nomake tests \
        -platform android-clang -android-sdk "$ANDROID_SDK_ROOT" -android-ndk "$ANDROID_NDK_ROOT" \
        -qt-host-path "$QT_PATH/host" -android-abis $abi
